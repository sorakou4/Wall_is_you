...
def charger_donjon(fichier):
    ...
    # La 2ème partie commence quand une ligne débute par A ou D
    indice = 0
    while indice < len(lignes) and not (lignes[indice].startswith("A") or lignes[indice].startswith("D")):
        indice += 1
...
def load_donjon(fichier="sauvegarde.txt"):
    ...
    donjon_charge, aventurier_charge, dragons_charge = charger_donjon(fichier)
