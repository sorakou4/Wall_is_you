...
- R = recommencer
- Échap = quitter
- S = sauvegarder le donjon
- L = charger un donjon sauvegardé
"""
import os
import moteur
import fltk
...
def ajouter_a_intention(case):
    """
    /!\ Obsolète : cette fonction n'est plus utilisée dans la version finale /!\
...
def appliquer_intention():
    ...
    # Vérifie si le trésor est trouvé
    if statut == "tresor":
        print("Vous êtes 🪙RICHE🪙", message)
        global compteur_tresors
        compteur_tresors = max(0, compteur_tresors - 1)
        fltk.texte(largeur_fenetre // 2, 20, message,couleur="yellow", ancrage="center", taille=20)
        fltk.mise_a_jour()
        fltk.attente(1)
        rafraichir_affichage()
        return
    
    # Vérifie immédiatement la défaite
    if statut == "defaite":
    ...
    if statut in ("en_cours", "victoire"):
        ...
        # Combat final après le déplacement complet
        # On vérifie si l’aventurier termine sur un dragon
        for d in dragons[:]:
            if d[0] == aventurier[0]:
                if aventurier[1] >= d[1]:
                    dragons.remove(d)
...
def nouvelle_partie(fichier="donjons/donjon_niv1.txt"):
    ...
    """
    global donjon, aventurier, dragons, bonus, intention, nb_lignes, nb_colonnes,donjon_indice,tresor
    moteur.tresor = []
