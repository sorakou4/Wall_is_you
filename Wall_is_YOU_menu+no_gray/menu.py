...
logo_path = None  # mettre "chemin/vers/logo.png" dans le main pour avoir le logo
...
def dessiner_menu(mx=None, my=None):
    ...
    # si logo_path défini, tentative d'afficher l'image 
    if logo_path:
        try:
            fltk.image(LARGEUR // 2, logo_y + logo_h // 2, logo_path, largeur=logo_w, hauteur=logo_h, ancrage="center")
        except Exception:
            pass
        ...
        if t == "Quitte":
            ...
            if afficher_sous_boutons:
                ...
                for idx, sb in enumerate(sous_boutons, start=1):
                    if sb.contient(x, y):
                        print(f"Appui sur sous-bouton {idx} (pas encore implémenté)")
                        # ici tu peux appeler une fonction de chargement
                        break
