...
import moteur
import fltk
from donjon_version_corrigee import deplacer_dragons_vers_aventurier
...
def rafraichir_affichage():
    ...
    fltk.texte(largeur_fenetre // 2, hauteur_fenetre - 30, f"Niveau de l'aventurier : {niveau_aventurier}", ancrage="center", taille=18, couleur="blue")

    afficher_commandes()
    # Bordure noire autour du donjon
    ...
    # Dessiner la grille
    marge_case = 0  # Espace entre les cases en pixels
    ...
    # Pour chaque dragon
    dragon_texture = 'texture/dragon.png'  # Chemin vers ton image
    for dragon in dragons:
        position, niveau = dragon
        cx, cy = centre_de_case(position)
        fltk.image(cx, cy, dragon_texture, largeur=int(taille_case * 0.6), hauteur=int(taille_case * 0.6), ancrage='center')
        rayon = int((taille_case - 4) * 0.38)  # Ajoute cette ligne !
        fltk.texte(cx, cy, str(niveau), couleur="red", ancrage="center", taille=max(8, rayon // 2))

    # Dessiner l'aventurier
    # Aventurier
    aventurier_texture = 'texture/aventurier.png'  # Chemin vers ton image
    pos_x, pos_y = centre_de_case(aventurier[0])
    fltk.image(pos_x, pos_y, aventurier_texture, largeur=int(taille_case*0.6), hauteur=int(taille_case*0.6), ancrage='center')
...
def appliquer_intention():
    ...
    if statut in ("en_cours", "victoire"):
        ...
        for position in chemin_restant:
            ...
            fltk.attente(0.18)
        dragons[:] = deplacer_dragons_vers_aventurier(donjon, dragons, aventurier)
        rafraichir_affichage()
...
"""le nouveau code nouvelle partie et programme principal pour les donjons//////////////////"""
def nouvelle_partie(fonction_creation_donjon):
    global donjon, aventurier, dragons, intention
    donjon, aventurier, dragons = fonction_creation_donjon()
    intention = []
    mettre_a_jour_taille_case()
    rafraichir_affichage()

def programme_principal(fonction_creation_donjon):
    global largeur_fenetre, hauteur_fenetre
    # Création de la fenêtre
    fltk.cree_fenetre(largeur_fenetre, hauteur_fenetre)
    nouvelle_partie(fonction_creation_donjon)
...
if __name__ == '__main__':
    """Lance le programme principal avec le donjon 1 par défaut//////////////////"""
    import moteur
    programme_principal(moteur.creer_donjon1)