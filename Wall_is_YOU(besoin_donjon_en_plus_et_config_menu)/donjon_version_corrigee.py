...
def deplacer_dragons_vers_aventurier(donjon, dragons, aventurier):
    nouvelles_positions = []
    pos_av = aventurier[0]
    for position, niveau in dragons:
        i, j = position
        iA, jA = pos_av
        # Cherche la meilleure case (adjacente) pour se rapprocher
        directions = []
        if iA < i:
        ...
        if jA > j:
            directions.append((i, j+1))
        bouge = position  # Par défaut il ne bouge pas
        for cible in directions:
            if (0 <= cible[0] < len(donjon)) and (0 <= cible[1] < len(donjon[0])):
                if sont_connectees(donjon, position, cible):
                    bouge = cible
                    break
        nouvelles_positions.append([bouge, niveau])
