...
def creer_donjon(nl, nc, nd, nb_bonus, graine=None):
    """
    ...
    # --- CORRECTION ICI : Création d'un chemin en "Serpentin" ---
    ...
    while k < len(salles) - 1:
        ...
        # On convertit en list pour pouvoir modifier (les tuples sont immuables)
        s1 = list(donjon[i1][j1])
        s2 = list(donjon[i2][j2])
        ...
        # On remet en tuple dans le donjon
        donjon[i1][j1] = tuple(s1)
        donjon[i2][j2] = tuple(s2)
        k += 1

    # Mélanger l’orientation des salles (rotation aléatoire)
    # Cela rend le jeu difficile (puzzle). Si vous voulez voir le chemin parfait tout de suite,
    # vous pouvez commenter ce bloc de rotation.
    for i in range(nl):
        for j in range(nc):
    ...
    dragons = []
    bonus = []
    # On utilise des tuples pour vérifier la présence dans occ, c'est plus sûr
    occ = {tuple(aventurier[0])}
    ...
    for _ in range(nb_bonus):
        while True:
            p = [random.randrange(nl), random.randrange(nc)]
            p_tuple = tuple(p)
            if p_tuple not in occ:
                occ.add(p_tuple)
                bonus.append([p, 0]) # Niveau toujours 0 pour le bonus
                break
...
def appliquer_tour_aventurier(donjon, aventurier, dragons, chemin, bonus):
    """
...
def trouver_bonus(bonus, pos):
    ...
    """
    pos_recherchee = tuple(pos)
    for i, b in enumerate(bonus):
        if tuple(b[0]) == pos_recherchee:
            return i
    return None
